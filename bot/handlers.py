...
from .setup import dish_searcher, user_service, dish_service
...
async def show_instruction_in_search_callback(callback: types.CallbackQuery,
    ...
    if callback.data == 'back':
        ...
        async with state.proxy() as data:
            ...
            user: UserDTO = get_cur_user(data)
            dish_service.save(dish)
            user_service.add_user_to_dish_association(dish=dish, user=user)
...
async def history_callback(callback: types.CallbackQuery, state: FSMContext):
    ...
    if callback.data == 'back':
        ...
        dish: DishDTO = dish_service.get(dish_id)
...
async def settings_callback(
    ...
    if callback.data == 'ru':
        lang = "ru"
    user_service.set_lang(get_chat_id(callback), lang=lang)
    await callback.message.delete()
    await to_start(update=callback, text=START)
